/**
 * Haseen UI — Theme Definitions
 *
 * Light and dark theme token maps using CSS custom properties.
 * All semantic tokens resolve to palette primitives + opacities.
 */

import { opacities, palette } from '../tokens';

const o = opacities;
const c = palette;

const misc = {
  '--hsn-primary-button-hover-shadow': '0 14px 34px rgba(16, 24, 40, 0.22)',
  '--hsn-primary-button-hover-inset-shadow': 'inset 0 1px 0 rgba(255, 255, 255, 0.18)',
  '--hsn-code-mark-color': `rgb(${c.teal500})`,
  '--hsn-code-mark-bg': `rgba(${c.black}, ${o[8]})`,
};

export interface ThemeTokens {
  [key: string]: string;
}

export interface ThemeSet {
  light: ThemeTokens;
  dark: ThemeTokens;
}

const colorVars = {
  '--hsn-white': c.white,
  '--hsn-black': c.black,
  '--hsn-grey-100': c.grey100,
  '--hsn-grey-200': c.grey200,
  '--hsn-grey-300': c.grey300,
  '--hsn-grey-400': c.grey400,
  '--hsn-grey-500': c.grey500,
  '--hsn-grey-600': c.grey600,
  '--hsn-grey-700': c.grey700,
  '--hsn-grey-800': c.grey800,
  '--hsn-grey-900': c.grey900,
  '--hsn-teal-100': c.teal100,
  '--hsn-teal-200': c.teal200,
  '--hsn-teal-300': c.teal300,
  '--hsn-teal-400': c.teal400,
  '--hsn-teal-500': c.teal500,
  '--hsn-teal-600': c.teal600,
  '--hsn-teal-700': c.teal700,
  '--hsn-teal-800': c.teal800,
  '--hsn-green-100': c.green100,
  '--hsn-green-200': c.green200,
  '--hsn-green-300': c.green300,
  '--hsn-green-400': c.green400,
  '--hsn-green-500': c.green500,
  '--hsn-green-600': c.green600,
  '--hsn-green-700': c.green700,
  '--hsn-green-800': c.green800,
  '--hsn-red-100': c.red100,
  '--hsn-red-200': c.red200,
  '--hsn-red-300': c.red300,
  '--hsn-red-400': c.red400,
  '--hsn-red-500': c.red500,
  '--hsn-red-600': c.red600,
  '--hsn-red-700': c.red700,
  '--hsn-red-800': c.red800,
  '--hsn-orange-100': c.orange100,
  '--hsn-orange-200': c.orange200,
  '--hsn-orange-300': c.orange300,
  '--hsn-orange-400': c.orange400,
  '--hsn-orange-500': c.orange500,
  '--hsn-orange-600': c.orange600,
  '--hsn-orange-700': c.orange700,
  '--hsn-orange-800': c.orange800,
  '--hsn-yellow-100': c.yellow100,
  '--hsn-yellow-200': c.yellow200,
  '--hsn-yellow-300': c.yellow300,
  '--hsn-yellow-400': c.yellow400,
  '--hsn-yellow-500': c.yellow500,
  '--hsn-yellow-600': c.yellow600,
  '--hsn-yellow-700': c.yellow700,
  '--hsn-yellow-800': c.yellow800,
  '--hsn-blue-100': c.blue100,
  '--hsn-blue-200': c.blue200,
  '--hsn-blue-300': c.blue300,
  '--hsn-blue-400': c.blue400,
  '--hsn-blue-500': c.blue500,
  '--hsn-blue-600': c.blue600,
  '--hsn-blue-700': c.blue700,
  '--hsn-blue-800': c.blue800,
  '--hsn-pink-100': c.pink100,
  '--hsn-pink-200': c.pink200,
  '--hsn-pink-300': c.pink300,
  '--hsn-pink-400': c.pink400,
  '--hsn-pink-500': c.pink500,
  '--hsn-pink-600': c.pink600,
  '--hsn-pink-700': c.pink700,
  '--hsn-pink-800': c.pink800,
  '--hsn-indigo-100': c.indigo100,
  '--hsn-indigo-200': c.indigo200,
  '--hsn-indigo-300': c.indigo300,
  '--hsn-indigo-400': c.indigo400,
  '--hsn-indigo-500': c.indigo500,
  '--hsn-indigo-600': c.indigo600,
  '--hsn-indigo-700': c.indigo700,
  '--hsn-indigo-800': c.indigo800,
};

const theme: ThemeSet = {
  light: {
    ...colorVars,
    ...misc,
    // ── Text ──
    '--hsn-text-primary': 'rgb(11, 18, 32)',
    '--hsn-text-secondary': 'rgba(18, 28, 45, 0.68)',
    '--hsn-text-tertiary': 'rgba(26, 37, 55, 0.48)',
    '--hsn-text-disabled': `rgba(${c.black}, ${o[32]})`,
    '--hsn-text-always-white': `rgb(${c.white})`,
    '--hsn-text-always-black': `rgb(${c.black})`,
    '--hsn-text-link': `rgb(${c.teal600})`,
    '--hsn-text-destructive': `rgb(${c.red600})`,
    '--hsn-text-inverse': `rgb(${c.white})`,
    // ── Icon ──
    '--hsn-icon-primary': `rgb(${c.grey900})`,
    '--hsn-icon-secondary': `rgba(${c.black}, ${o[56]})`,
    '--hsn-icon-tertiary': `rgba(${c.black}, ${o[42]})`,
    '--hsn-icon-disabled': `rgba(${c.black}, ${o[32]})`,
    '--hsn-icon-always-white': `rgb(${c.white})`,
    '--hsn-icon-always-black': `rgb(${c.black})`,
    '--hsn-icon-link': `rgb(${c.teal600})`,
    '--hsn-icon-destructive': `rgb(${c.red600})`,
    '--hsn-icon-inverse': `rgb(${c.white})`,
    // ── CTA / Button ──
    '--hsn-cta-primary-default': 'linear-gradient(135deg, rgb(45, 184, 175) 0%, rgb(63, 146, 255) 100%)',
    '--hsn-cta-primary-hover': 'linear-gradient(135deg, rgb(34, 168, 160) 0%, rgb(45, 122, 226) 100%)',
    '--hsn-cta-primary-active': 'linear-gradient(135deg, rgb(23, 131, 125) 0%, rgb(30, 94, 185) 100%)',
    '--hsn-cta-primary-disabled': `rgba(${c.teal500}, ${o[32]})`,
    '--hsn-cta-secondary-default': 'rgba(255, 255, 255, 0.82)',
    '--hsn-cta-secondary-hover': 'rgba(248, 250, 252, 0.96)',
    '--hsn-cta-secondary-active': 'rgba(241, 245, 249, 0.98)',
    '--hsn-cta-secondary-disabled': 'transparent',
    '--hsn-cta-tertiary-default': 'transparent',
    '--hsn-cta-tertiary-hover': `rgba(${c.black}, ${o[8]})`,
    '--hsn-cta-tertiary-active': `rgba(${c.black}, ${o[10]})`,
    '--hsn-cta-tertiary-disabled': 'transparent',
    '--hsn-cta-destructive-default': 'transparent',
    '--hsn-cta-destructive-hover': `rgba(${c.red600}, ${o[8]})`,
    '--hsn-cta-destructive-active': `rgba(${c.red600}, ${o[12]})`,
    '--hsn-cta-destructive-disabled': 'transparent',
    '--hsn-cta-chip-default': 'transparent',
    '--hsn-cta-chip-hover': `rgba(${c.black}, ${o[6]})`,
    '--hsn-cta-navigation-default': 'transparent',
    '--hsn-cta-navigation-hover': `rgba(${c.black}, ${o[6]})`,
    '--hsn-cta-navigation-active': `rgba(${c.black}, ${o[12]})`,
    '--hsn-cta-navigation-disabled': 'transparent',
    // ── Border ──
    '--hsn-border-primary': 'rgba(15, 23, 42, 0.1)',
    '--hsn-border-secondary': 'rgba(15, 23, 42, 0.08)',
    '--hsn-border-tertiary': 'rgba(148, 163, 184, 0.12)',
    '--hsn-border-hover': 'rgba(45, 184, 175, 0.42)',
    '--hsn-border-active': `rgb(${c.teal500})`,
    '--hsn-border-destructive': `rgba(${c.red600}, ${o[12]})`,
    // ── Background ──
    '--hsn-bg-main-container': 'radial-gradient(circle at top left, rgba(45, 184, 175, 0.12), transparent 32%), radial-gradient(circle at top right, rgba(99, 102, 241, 0.1), transparent 28%), linear-gradient(180deg, #f8fbff 0%, #eef4fb 100%)',
    '--hsn-bg-sidepanel': 'linear-gradient(180deg, rgba(255, 255, 255, 0.94), rgba(245, 249, 255, 0.9))',
    '--hsn-bg-l0-solid': 'linear-gradient(180deg, rgba(255, 255, 255, 0.94), rgba(248, 251, 255, 0.9))',
    '--hsn-bg-l0-glass': 'rgba(255, 255, 255, 0.72)',
    '--hsn-bg-l1-solid': 'linear-gradient(180deg, rgba(255, 255, 255, 0.98), rgba(246, 250, 255, 0.96))',
    '--hsn-bg-l1-glass': 'rgba(255, 255, 255, 0.78)',
    '--hsn-bg-l2-solid': 'linear-gradient(180deg, rgba(255, 255, 255, 1), rgba(249, 251, 255, 0.98))',
    '--hsn-bg-l2-glass': 'rgba(255, 255, 255, 0.82)',
    '--hsn-bg-l3-solid': 'linear-gradient(180deg, rgba(255, 255, 255, 1), rgba(252, 253, 255, 0.98))',
    '--hsn-bg-l3-glass': 'rgba(255, 255, 255, 0.88)',
    '--hsn-bg-emphasis': `rgb(${c.grey900})`,
    '--hsn-bg-scrim': 'rgba(241, 245, 249, 0.78)',
    '--hsn-bg-cell-hover': 'rgba(15, 23, 42, 0.05)',
    '--hsn-bg-cell-active': 'rgba(15, 23, 42, 0.08)',
    '--hsn-bg-field-default': 'rgba(248, 250, 252, 0.9)',
    '--hsn-bg-field-hover': 'rgba(255, 255, 255, 0.98)',
    // ── Overlay ──
    '--hsn-bg-overlay-primary': `rgba(${c.black}, ${o[10]})`,
    '--hsn-bg-overlay-secondary': `rgba(${c.black}, ${o[8]})`,
    '--hsn-bg-overlay-tertiary': `rgba(${c.black}, ${o[4]})`,
    '--hsn-bg-overlay-destructive': `rgba(${c.red400}, ${o[24]})`,
    // ── Accent ──
    '--hsn-accent-teal-primary': `rgb(${c.teal600})`,
    '--hsn-accent-teal-secondary': `rgba(${c.teal400}, ${o[36]})`,
    '--hsn-accent-green-primary': `rgb(${c.green600})`,
    '--hsn-accent-green-secondary': `rgba(${c.green400}, ${o[28]})`,
    '--hsn-accent-orange-primary': `rgb(${c.orange600})`,
    '--hsn-accent-orange-secondary': `rgba(${c.orange400}, ${o[36]})`,
    '--hsn-accent-pink-primary': `rgb(${c.pink600})`,
    '--hsn-accent-pink-secondary': `rgba(${c.pink400}, ${o[36]})`,
    '--hsn-accent-yellow-primary': `rgb(${c.yellow600})`,
    '--hsn-accent-yellow-secondary': `rgba(${c.yellow400}, ${o[48]})`,
    '--hsn-accent-blue-primary': `rgb(${c.blue600})`,
    '--hsn-accent-blue-secondary': `rgba(${c.blue400}, ${o[36]})`,
    '--hsn-accent-red-primary': `rgb(${c.red600})`,
    '--hsn-accent-red-secondary': `rgba(${c.red400}, ${o[36]})`,
    '--hsn-accent-indigo-primary': `rgb(${c.indigo600})`,
    '--hsn-accent-indigo-secondary': `rgba(${c.indigo400}, ${o[36]})`,
    // ── Shadows ──
    '--hsn-shadow-l1': '0 18px 40px -28px rgba(15, 23, 42, 0.22), 0 8px 20px -18px rgba(15, 23, 42, 0.18)',
    '--hsn-shadow-l2': '0 28px 60px -34px rgba(15, 23, 42, 0.3), 0 14px 26px -22px rgba(15, 23, 42, 0.16)',
    '--hsn-shadow-l3': '0 44px 100px -40px rgba(15, 23, 42, 0.4), 0 22px 46px -28px rgba(15, 23, 42, 0.26)',
    '--hsn-shadow-inset': 'inset 0 1px 0 rgba(255, 255, 255, 0.8)',
    '--hsn-shadow-secondary-button': '0 10px 24px -18px rgba(15, 23, 42, 0.2), inset 0 1px 0 rgba(255, 255, 255, 0.9)',
    '--hsn-shadow-destructive-button': 'inset 0px -1px 0px 1px rgba(220, 38, 38, 0.2), inset 0px 1px 0px rgba(220, 38, 38, 0.2)',
    '--hsn-shadow-dropdown': 'rgba(27, 31, 35, 0.04) 0 1px 0, rgba(255, 255, 255, 0.25) 0 2px 0 inset',
    '--hsn-shadow-drawer': '0px 0px 40px rgba(0, 0, 0, 0.1)',
    // ── Misc ──
    '--hsn-bg-cell-unread': `rgb(${c.white})`,
    '--hsn-status-bar-color': `linear-gradient(0deg, rgba(${c.black}, ${o[2]}), rgba(${c.black}, ${o[2]})), rgb(${c.white})`,
    '--hsn-illustration-fill': `rgb(${c.grey100})`,
  },
  dark: {
    ...colorVars,
    ...misc,
    // ── Text ──
    '--hsn-text-primary': 'rgb(240, 247, 255)',
    '--hsn-text-secondary': 'rgba(223, 234, 248, 0.72)',
    '--hsn-text-tertiary': 'rgba(185, 199, 218, 0.58)',
    '--hsn-text-disabled': `rgba(${c.white}, ${o[28]})`,
    '--hsn-text-always-white': `rgb(${c.white})`,
    '--hsn-text-always-black': `rgb(${c.black})`,
    '--hsn-text-link': `rgb(${c.teal400})`,
    '--hsn-text-destructive': `rgb(${c.red400})`,
    '--hsn-text-inverse': `rgb(${c.black})`,
    // ── Icon ──
    '--hsn-icon-primary': `rgb(${c.white})`,
    '--hsn-icon-secondary': `rgba(${c.white}, ${o[54]})`,
    '--hsn-icon-tertiary': `rgba(${c.white}, ${o[42]})`,
    '--hsn-icon-disabled': `rgba(${c.white}, ${o[28]})`,
    '--hsn-icon-always-white': `rgb(${c.white})`,
    '--hsn-icon-always-black': `rgb(${c.black})`,
    '--hsn-icon-link': `rgb(${c.teal400})`,
    '--hsn-icon-destructive': `rgb(${c.red400})`,
    '--hsn-icon-inverse': `rgb(${c.black})`,
    // ── CTA / Button ──
    '--hsn-cta-primary-default': 'linear-gradient(135deg, rgb(45, 184, 175) 0%, rgb(77, 145, 255) 100%)',
    '--hsn-cta-primary-hover': 'linear-gradient(135deg, rgb(72, 210, 199) 0%, rgb(107, 168, 255) 100%)',
    '--hsn-cta-primary-active': 'linear-gradient(135deg, rgb(38, 157, 149) 0%, rgb(62, 125, 231) 100%)',
    '--hsn-cta-primary-disabled': `rgba(${c.teal500}, ${o[32]})`,
    '--hsn-cta-secondary-default': 'rgba(15, 23, 42, 0.42)',
    '--hsn-cta-secondary-hover': 'rgba(24, 35, 55, 0.56)',
    '--hsn-cta-secondary-active': 'rgba(30, 41, 59, 0.72)',
    '--hsn-cta-secondary-disabled': 'transparent',
    '--hsn-cta-tertiary-default': 'transparent',
    '--hsn-cta-tertiary-hover': `rgba(${c.white}, ${o[8]})`,
    '--hsn-cta-tertiary-active': `rgba(${c.white}, ${o[10]})`,
    '--hsn-cta-tertiary-disabled': 'transparent',
    '--hsn-cta-destructive-default': `rgba(${c.white}, ${o[4]})`,
    '--hsn-cta-destructive-hover': `rgba(${c.red400}, ${o[12]})`,
    '--hsn-cta-destructive-active': `rgba(${c.red400}, ${o[16]})`,
    '--hsn-cta-destructive-disabled': 'transparent',
    '--hsn-cta-chip-default': 'transparent',
    '--hsn-cta-chip-hover': `rgba(${c.white}, ${o[8]})`,
    '--hsn-cta-navigation-default': 'transparent',
    '--hsn-cta-navigation-hover': `rgba(${c.white}, ${o[8]})`,
    '--hsn-cta-navigation-active': `rgba(${c.white}, ${o[12]})`,
    '--hsn-cta-navigation-disabled': 'transparent',
    // ── Border ──
    '--hsn-border-primary': 'rgba(148, 163, 184, 0.2)',
    '--hsn-border-secondary': 'rgba(148, 163, 184, 0.14)',
    '--hsn-border-tertiary': 'rgba(255, 255, 255, 0.06)',
    '--hsn-border-hover': 'rgba(45, 184, 175, 0.5)',
    '--hsn-border-active': `rgb(${c.teal400})`,
    '--hsn-border-destructive': `rgba(${c.red400}, ${o[12]})`,
    // ── Background ──
    '--hsn-bg-main-container': 'radial-gradient(circle at top left, rgba(45, 184, 175, 0.16), transparent 28%), radial-gradient(circle at top right, rgba(96, 165, 250, 0.18), transparent 26%), linear-gradient(180deg, #030712 0%, #08111f 42%, #0d1728 100%)',
    '--hsn-bg-sidepanel': 'linear-gradient(180deg, rgba(9, 16, 28, 0.92), rgba(12, 22, 38, 0.92))',
    '--hsn-bg-l0-solid': 'linear-gradient(180deg, rgba(12, 21, 36, 0.84), rgba(8, 15, 27, 0.9))',
    '--hsn-bg-l0-glass': 'rgba(8, 15, 27, 0.66)',
    '--hsn-bg-l1-solid': 'linear-gradient(180deg, rgba(13, 23, 39, 0.92), rgba(9, 17, 31, 0.92))',
    '--hsn-bg-l1-glass': 'rgba(12, 22, 38, 0.72)',
    '--hsn-bg-l2-solid': 'linear-gradient(180deg, rgba(16, 28, 45, 0.96), rgba(11, 20, 35, 0.96))',
    '--hsn-bg-l2-glass': 'rgba(16, 28, 45, 0.76)',
    '--hsn-bg-l3-solid': 'linear-gradient(180deg, rgba(20, 35, 56, 0.98), rgba(13, 23, 40, 0.98))',
    '--hsn-bg-l3-glass': 'rgba(20, 35, 56, 0.82)',
    '--hsn-bg-emphasis': 'rgb(20, 35, 56)',
    '--hsn-bg-scrim': 'rgba(2, 6, 23, 0.68)',
    '--hsn-bg-cell-hover': 'rgba(148, 163, 184, 0.1)',
    '--hsn-bg-cell-active': 'rgba(148, 163, 184, 0.14)',
    '--hsn-bg-field-default': 'rgba(10, 18, 32, 0.82)',
    '--hsn-bg-field-hover': 'rgba(16, 28, 45, 0.92)',
    // ── Overlay ──
    '--hsn-bg-overlay-primary': `rgba(${c.white}, ${o[10]})`,
    '--hsn-bg-overlay-secondary': `rgba(${c.white}, ${o[8]})`,
    '--hsn-bg-overlay-tertiary': `rgba(${c.white}, ${o[4]})`,
    '--hsn-bg-overlay-destructive': `rgba(${c.red400}, ${o[24]})`,
    // ── Accent ──
    '--hsn-accent-teal-primary': `rgba(${c.teal400}, ${o[88]})`,
    '--hsn-accent-teal-secondary': `rgba(${c.teal400}, ${o[36]})`,
    '--hsn-accent-green-primary': `rgba(${c.green400}, ${o[88]})`,
    '--hsn-accent-green-secondary': `rgba(${c.green400}, ${o[32]})`,
    '--hsn-accent-orange-primary': `rgba(${c.orange400}, ${o[88]})`,
    '--hsn-accent-orange-secondary': `rgba(${c.orange400}, ${o[36]})`,
    '--hsn-accent-pink-primary': `rgba(${c.pink400}, ${o[88]})`,
    '--hsn-accent-pink-secondary': `rgba(${c.pink400}, ${o[36]})`,
    '--hsn-accent-yellow-primary': `rgba(${c.yellow400}, ${o[88]})`,
    '--hsn-accent-yellow-secondary': `rgba(${c.yellow400}, ${o[36]})`,
    '--hsn-accent-blue-primary': `rgba(${c.blue400}, ${o[88]})`,
    '--hsn-accent-blue-secondary': `rgba(${c.blue400}, ${o[36]})`,
    '--hsn-accent-red-primary': `rgba(${c.red400}, ${o[88]})`,
    '--hsn-accent-red-secondary': `rgba(${c.red400}, ${o[36]})`,
    '--hsn-accent-indigo-primary': `rgba(${c.indigo400}, ${o[88]})`,
    '--hsn-accent-indigo-secondary': `rgba(${c.indigo400}, ${o[36]})`,
    // ── Shadows ──
    '--hsn-shadow-l1': '0 22px 52px -34px rgba(2, 6, 23, 0.75), 0 10px 22px -16px rgba(8, 15, 27, 0.54)',
    '--hsn-shadow-l2': '0 34px 74px -38px rgba(2, 6, 23, 0.82), 0 18px 34px -20px rgba(8, 15, 27, 0.62)',
    '--hsn-shadow-l3': '0 52px 110px -42px rgba(2, 6, 23, 0.9), 0 24px 48px -24px rgba(8, 15, 27, 0.7)',
    '--hsn-shadow-inset': 'inset 0 1px 0 rgba(255, 255, 255, 0.08)',
    '--hsn-shadow-secondary-button': '0 18px 32px -24px rgba(2, 6, 23, 0.72), inset 0 1px 0 rgba(255, 255, 255, 0.08)',
    '--hsn-shadow-destructive-button': 'inset 0px -1px 0px 1px rgba(248, 113, 113, 0.12), inset 0px 1px 0px rgba(248, 113, 113, 0.12)',
    '--hsn-shadow-dropdown': 'rgba(27, 31, 35, 0.04) 0 1px 0, rgba(255, 255, 255, 0.25) 0 2px 0 inset',
    '--hsn-shadow-drawer': '0px 0px 40px rgba(0, 0, 0, 0.1)',
    // ── Misc ──
    '--hsn-bg-cell-unread': `linear-gradient(0deg, rgba(${c.white}, ${o[14]}), rgba(${c.white}, ${o[14]})), rgb(${c.black})`,
    '--hsn-status-bar-color': `linear-gradient(0deg, rgba(${c.white}, ${o[12]}), rgba(${c.white}, ${o[12]})), rgb(${c.black})`,
    '--hsn-illustration-fill': `rgb(${c.grey800})`,
  },
};

export const themeNames = theme;
